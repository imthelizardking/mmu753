...
Lkr = 0.01; L1 = 0.3534; L2 = 90.9; NFFT = 1024; q  = 100;
...
% Road classes to sweep
Gh0_sweep = [(4.8e-7)/(2*pi);   % Smooth highway
             (2.0e-6)/(2*pi);   % Average road  
             (4.39e-6)/(2*pi);  % Minor road
             (1.5e-5)/(2*pi)];  % Poor road
Gh0 = Gh0_sweep(1); % Smooth highway
% Gh0 = Gh0_sweep(2); % Average road
% Gh0 = Gh0_sweep(3); % Minor road
% Gh0 = Gh0_sweep(4); % Poor road
